...
/**
 ...
 Create instances of ``NGLPointAnnotation``, ``NGLPointAnnotation``, ``NGLPointAnnotation``,
 ``NGLMultiPolyline``, ``NGLMultiPolyline``, ``NGLMultiPolyline``, or ``NGLMultiPolyline`` in
 ...
 Configure the appearance of an ``NGLShapeSource``’s or ``NGLShapeSource``’s
...
/**
 ...
 ``NGLShape`` that conforms to the ``NGLShape`` protocol. If it is a feature
 ...
 let feature = try! NGLShape(data: data, encoding: String.Encoding.utf8.rawValue) as!
 NGLShapeCollectionFeature
 ```
 ...
 */
+ (nullable NGLShape *)shapeWithData:(NSData *)data
