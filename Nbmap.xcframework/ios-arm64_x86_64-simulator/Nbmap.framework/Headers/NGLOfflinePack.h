...
FOUNDATION_EXTERN NGL_EXPORT NGLExceptionName const NGLInvalidOfflinePackException;

/**
 The state an offline pack is currently in.
 */
typedef NS_ENUM(NSInteger, NGLOfflinePackState) {
  /**
   ...
   the time the shared ``NGLOfflineStorage`` object sends the first
   ``NGLOfflinePackProgressChangedNotification`` about the pack. For inactive
   packs, you must explicitly request a progress update using the
   ``NGLOfflinePack/requestProgress`` method.

   An invalid pack always has a state of ``NGLOfflinePackStateInvalid``, never
   ``NGLOfflinePackState/NGLOfflinePackStateUnknown``.
   */
  NGLOfflinePackStateUnknown = 0,
  /**
   The pack is incomplete and is not currently downloading.

   This is the initial state of a pack that is created using the
   ``NGLOfflineStorage/addPackForRegion:withContext:completionHandler:``
   method, as well as after the ``NGLOfflinePack/suspend`` method is
   called.
   */
  NGLOfflinePackStateInactive = 1,
  /**
   The pack is incomplete and is currently downloading.

   This is the state of a pack after the ``NGLOfflinePack/resume`` method is
   called.
   */
  NGLOfflinePackStateActive = 2,
  /**
   The pack has downloaded to completion.
   */
  NGLOfflinePackStateComplete = 3,
  /**
   The pack has been removed using the
   ``NGLOfflineStorage/removePack:withCompletionHandler:`` method. Sending
   any message to the pack will raise an exception.
   */
  NGLOfflinePackStateInvalid = 4,
};
...
/**
 ...
 progress.
 */
typedef struct __attribute__((objc_boxable)) NGLOfflinePackProgress {
  /**
...
} NGLOfflinePackProgress;

/**
 An ``NGLOfflinePack`` represents a collection of resources necessary for viewing
 a region offline to a local database.

 To create an instance of ``NGLOfflinePack``, use the
 ``NGLOfflineStorage/addPackForRegion:withContext:completionHandler:`` method.
 A pack created using `NGLOfflinePack/init` is immediately invalid.

 ### Example
 ```swift
 NGLOfflineStorage.shared.addPack(for: region, withContext: context) { (pack, error) in
 ...
 \(#file)(\(#line)) in \(#function)") return
     ...
     // Start an NGLOfflinePack download
     pack.resume()
 }
 ```
 */
NGL_EXPORT
@interface NGLOfflinePack : NSObject

/**
 The region for which the pack manages resources.
 */
@property (nonatomic, readonly) id<NGLOfflineRegion> region;
...
/**
 ...
 ``NGLOfflinePackState/NGLOfflinePackStateUnknown`` by default. To request the pack’s status, use
 the
 ...
 ``NGLOfflinePackProgressChangedNotification``s about this pack that come from the
 default notification center.
 */
@property (nonatomic, readonly) NGLOfflinePackState state;
...
/**
 ...
 default notification center.
 */
@property (nonatomic, readonly) NGLOfflinePackProgress progress;
...
/**
 ...
 set to ``NGLOfflinePackState/NGLOfflinePackStateUnknown``, observe KVO change notifications on this
 pack’s `state` key path, then call this method. Alternatively, you can add an
 observer for ``NGLOfflinePackProgressChangedNotification`` about this pack that
