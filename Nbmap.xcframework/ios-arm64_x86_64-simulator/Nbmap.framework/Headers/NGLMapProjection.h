...
*/
NGL_EXPORT
@interface NGLMapProjection : NSObject
 ...
 */
- (instancetype)initWithMapView:(NGLMapView *)mapView;

/**
 A camera representing the current projection state
 */
@property (readonly, copy) NGLMapCamera *camera;
...
/**
 ...
 > Note: ``NGLMapView`` instance frame must not be changed since this projection is initialized,
        otherwise the calculation may be wrong.
 */
- (void)setCamera:(NGLMapCamera *_Nonnull)camera withEdgeInsets:(UIEdgeInsets)insets;
...
/**
 ...
 > Note: ``NGLMapView`` instance frame must not be changed since this projection is initialized,
     otherwise the calculation may be wrong.
 */
- (void)setVisibleCoordinateBounds:(NGLCoordinateBounds)bounds edgePadding:(UIEdgeInsets)insets;
