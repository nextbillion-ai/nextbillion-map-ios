...
/// A structure containing context needed to draw a frame in an ``NGLCustomStyleLayer``.
...
///
/// You can access an existing ``NGLCustomStyleLayer`` using the
/// ``NGLStyle/layerWithIdentifier:`` method if you know its identifier;
/// otherwise, find it using the ``NGLStyle/layers`` property. You can also
/// create a new ``NGLCustomStyleLayer`` and add it to the style using a method such as
/// ``NGLStyle/addLayer:``.
///
/// - Warning: This API experimental. It may change
///   at any time without notice.
NGL_EXPORT
@interface NGLCustomStyleLayer : NGLStyleLayer
...
/// Returns an ``NGLCustomStyleLayer`` style layer object initialized with the given identifier.
///
/// After initializing and configuring the style layer, add it to a map view’s style
/// using the ``NGLStyle/addLayer:`` or
