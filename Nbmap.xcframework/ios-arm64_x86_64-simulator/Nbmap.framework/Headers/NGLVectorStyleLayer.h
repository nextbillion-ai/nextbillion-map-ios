...
/**
 ...
 is defined by an ``NGLShapeSource`` or ``NGLShapeSource`` object.

 Create instances of ``NGLCircleStyleLayer``, ``NGLCircleStyleLayer``,
 ``NGLFillExtrusionStyleLayer``, ``NGLFillExtrusionStyleLayer``, ``NGLFillExtrusionStyleLayer``, and
 ``NGLSymbolStyleLayer`` in order to use ``NGLSymbolStyleLayer``'s properties and
