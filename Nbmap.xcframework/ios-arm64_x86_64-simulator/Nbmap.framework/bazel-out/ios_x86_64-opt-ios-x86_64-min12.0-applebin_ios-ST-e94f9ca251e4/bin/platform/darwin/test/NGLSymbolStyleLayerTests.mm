...
#include "../../darwin/src/NGLAttributedExpression.h"
...
@interface NGLSymbolLayerTests : NGLStyleLayerTests
@end

@implementation NGLSymbolLayerTests
...
- (void)testPredicates {
    ...
    NGLSymbolStyleLayer *layer = [[NGLSymbolStyleLayer alloc] initWithIdentifier:@"layerID" source:source];
...
- (void)testProperties {
    ...
    auto rawLayer = static_cast<nbgl::style::SymbolLayer*>(layer.rawLayer);
    ...
    // icon-allow-overlap
    {
        ...
        nbgl::style::PropertyValue<bool> propertyValue = { true };
        ...
        XCTAssertThrowsSpecificNamed(layer.iconAllowsOverlap = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    ...
    // icon-anchor
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::SymbolAnchorType> propertyValue = { nbgl::style::SymbolAnchorType::BottomRight };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::SymbolAnchorType>(
    ...
    // icon-ignore-placement
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.iconIgnoresPlacement = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.iconIgnoresPlacement = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // icon-image
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::expression::Image> propertyValue = { "Icon Image" };
        ...
        constantExpression = [NSExpression expressionWithFormat:@"NGL_FUNCTION('image', 'Icon Image')"];
        XCTExpectFailure(@"Awaiting unit test refactoring https://github.com/maplibre/maplibre-native/issues/421");
        NSExpression *functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:($zoomLevel, %@, %@)", constantExpression, @{@18: constantExpression}];
        ...
        NSExpression* tokenExpression = [NSExpression expressionWithFormat:@"NGL_FUNCTION('image', CAST(token, \"NSString\"))"];
    ...
    // icon-optional
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.iconOptional = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.iconOptional = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // icon-padding
    {
        ...
        nbgl::style::PropertyValue<nbgl::Padding> propertyValue = { { 1, 1, 1, 1 } };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::Padding>(
...

        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::Padding>(
                interpolate(linear(), number(get("keyName")), 18.0, literal({ 1, 1, 1, 1 }))
            );
        }

        XCTAssertEqual(rawLayer->getIconPadding(), propertyValue,
                       @"Setting iconPadding to a data expression should update icon-padding.");
        NSExpression *pedanticFunctionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:(CAST(keyName, 'NSNumber'), 'linear', nil, %@)", @{@18: constantExpression}];
        XCTAssertEqualObjects(layer.iconPadding, pedanticFunctionExpression,
                              @"iconPadding should round-trip data expressions.");
...

        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::Padding>(
                interpolate(linear(), zoom(), 10.0, interpolate(linear(), number(get("keyName")), 18.0, literal({ 1, 1, 1, 1 })))
            );
        }

        XCTAssertEqual(rawLayer->getIconPadding(), propertyValue,
                       @"Setting iconPadding to a camera-data expression should update icon-padding.");
        pedanticFunctionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: pedanticFunctionExpression}];
        XCTAssertEqualObjects(layer.iconPadding, pedanticFunctionExpression,
                              @"iconPadding should round-trip camera-data expressions.");
    ...
    // icon-pitch-alignment
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::AlignmentType> propertyValue = { nbgl::style::AlignmentType::Auto };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::AlignmentType>(
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.iconPitchAlignment = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.iconPitchAlignment = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // icon-rotation-alignment
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.iconRotationAlignment = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.iconRotationAlignment = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // icon-text-fit
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::IconTextFitType> propertyValue = { nbgl::style::IconTextFitType::Both };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::IconTextFitType>(
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.iconTextFit = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.iconTextFit = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // icon-text-fit-padding
    {
        ...
        nbgl::style::PropertyValue<std::array<float, 4>> propertyValue = { { 1, 1, 1, 1 } };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<std::array<float, 4>>(
                step(zoom(), literal({ 1, 1, 1, 1 }), 18.0, literal({ 1, 1, 1, 1 }))
            );
        }
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.iconTextFitPadding = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.iconTextFitPadding = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // icon-keep-upright
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.keepsIconUpright = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.keepsIconUpright = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-keep-upright
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.keepsTextUpright = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.keepsTextUpright = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-max-angle
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.maximumTextAngle = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.maximumTextAngle = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // symbol-avoid-edges
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.symbolAvoidsEdges = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.symbolAvoidsEdges = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // symbol-placement
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::SymbolPlacementType> propertyValue = { nbgl::style::SymbolPlacementType::LineCenter };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::SymbolPlacementType>(
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.symbolPlacement = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.symbolPlacement = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // symbol-screen-space
    {
        XCTAssertTrue(rawLayer->getSymbolScreenSpace().isUndefined(),
                      @"symbol-screen-space should be unset initially.");
        NSExpression *defaultExpression = layer.symbolScreenSpace;

        NSExpression *constantExpression = [NSExpression expressionWithFormat:@"true"];
        layer.symbolScreenSpace = constantExpression;
        nbgl::style::PropertyValue<bool> propertyValue = { true };
        XCTAssertEqual(rawLayer->getSymbolScreenSpace(), propertyValue,
                       @"Setting symbolScreenSpace to a constant value expression should update symbol-screen-space.");
        XCTAssertEqualObjects(layer.symbolScreenSpace, constantExpression,
                              @"symbolScreenSpace should round-trip constant value expressions.");

        constantExpression = [NSExpression expressionWithFormat:@"true"];
        XCTExpectFailure(@"Awaiting unit test refactoring https://github.com/maplibre/maplibre-native/issues/421");
        NSExpression *functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:($zoomLevel, %@, %@)", constantExpression, @{@18: constantExpression}];
        layer.symbolScreenSpace = functionExpression;
        ...
        XCTAssertEqual(rawLayer->getSymbolScreenSpace(), propertyValue,
                       @"Setting symbolScreenSpace to a camera expression should update symbol-screen-space.");
        XCTAssertEqualObjects(layer.symbolScreenSpace, functionExpression,
                              @"symbolScreenSpace should round-trip camera expressions.");


        layer.symbolScreenSpace = nil;
        XCTAssertTrue(rawLayer->getSymbolScreenSpace().isUndefined(),
                      @"Unsetting symbolScreenSpace should return symbol-screen-space to the default value.");
        XCTAssertEqualObjects(layer.symbolScreenSpace, defaultExpression,
                              @"symbolScreenSpace should return the default value after being unset.");

        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.symbolScreenSpace = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.symbolScreenSpace = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // symbol-spacing
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.symbolSpacing = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.symbolSpacing = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // symbol-z-order
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::SymbolZOrderType> propertyValue = { nbgl::style::SymbolZOrderType::Source };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::SymbolZOrderType>(
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.symbolZOrder = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.symbolZOrder = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-field
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::expression::Formatted> propertyValue = { "Text Field" };
        ...
        NGLAttributedExpression *attributedConstantExpression = [[NGLAttributedExpression alloc] initWithExpression:[NSExpression expressionWithFormat:@"'Text Field'"]
                                                                                                 attributes:@{}];
        constantExpression = [NSExpression ngl_expressionForAttributedExpressions:@[[NSExpression expressionForConstantValue:attributedConstantExpression]]];
        XCTExpectFailure(@"Awaiting unit test refactoring https://github.com/maplibre/maplibre-native/issues/421");
        NSExpression *functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:($zoomLevel, %@, %@)", constantExpression, @{@18: constantExpression}];
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::expression::Formatted>(
        ...
        NGLAttributedExpression *tokenAttibutedExpression = [[NGLAttributedExpression alloc] initWithExpression:[NSExpression expressionWithFormat:@"CAST(token, 'NSString')"]
                                                                                                     attributes:@{}];
        NSExpression* tokenExpression = [NSExpression ngl_expressionForAttributedExpressions:@[[NSExpression expressionForConstantValue:tokenAttibutedExpression]]];
    ...
    // text-allow-overlap
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textAllowsOverlap = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textAllowsOverlap = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-font
    {
        ...
        nbgl::style::PropertyValue<std::vector<std::string>> propertyValue = { { "Text Font", "Tnof Txet" } };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<std::vector<std::string>>(
    ...
    // text-ignore-placement
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textIgnoresPlacement = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textIgnoresPlacement = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-justify
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::TextJustifyType> propertyValue = { nbgl::style::TextJustifyType::Right };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::TextJustifyType>(
    ...
    // text-line-height
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textLineHeight = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textLineHeight = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-optional
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textOptional = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textOptional = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-padding
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textPadding = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textPadding = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-pitch-alignment
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textPitchAlignment = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textPitchAlignment = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-rotation-alignment
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textRotationAlignment = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textRotationAlignment = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-transform
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::TextTransformType> propertyValue = { nbgl::style::TextTransformType::Lowercase };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::TextTransformType>(
    ...
    // text-variable-anchor
    {
        ...
        nbgl::style::PropertyValue<std::vector<nbgl::style::SymbolAnchorType>> propertyValue = { { nbgl::style::SymbolAnchorType::Top, nbgl::style::SymbolAnchorType::Bottom } };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<std::vector<nbgl::style::SymbolAnchorType>>(
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textVariableAnchor = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textVariableAnchor = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    {
        XCTAssertTrue(rawLayer->getTextVariableAnchorOffset().isUndefined(),
                      @"text-variable-anchor-offset should be unset initially.");
        NSExpression *defaultExpression = layer.textVariableAnchorOffset;

        NSExpression *constantExpression = [NSExpression expressionWithFormat:@"{"top": [1, 2]}"];
        layer.textVariableAnchorOffset = constantExpression;
        nbgl::style::PropertyValue<nbgl::VariableAnchorOffsetCollection> propertyValue = { "Text Variable Anchor Offset" };
        XCTAssertEqual(rawLayer->getTextVariableAnchorOffset(), propertyValue,
                       @"Setting textVariableAnchorOffset to a constant value expression should update text-variable-anchor-offset.");
        XCTAssertEqualObjects(layer.textVariableAnchorOffset, constantExpression,
                              @"textVariableAnchorOffset should round-trip constant value expressions.");

        constantExpression = [NSExpression expressionWithFormat:@"{"top": [1, 2]}"];
        XCTExpectFailure(@"Awaiting unit test refactoring https://github.com/maplibre/maplibre-native/issues/421");
        NSExpression *functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:($zoomLevel, %@, %@)", constantExpression, @{@18: constantExpression}];
        layer.textVariableAnchorOffset = functionExpression;

        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::VariableAnchorOffsetCollection>(
                step(zoom(), literal("Text Variable Anchor Offset"), 18.0, literal("Text Variable Anchor Offset"))
            );
        }

        XCTAssertEqual(rawLayer->getTextVariableAnchorOffset(), propertyValue,
                       @"Setting textVariableAnchorOffset to a camera expression should update text-variable-anchor-offset.");
        XCTAssertEqualObjects(layer.textVariableAnchorOffset, functionExpression,
                              @"textVariableAnchorOffset should round-trip camera expressions.");
...

        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::VariableAnchorOffsetCollection>(
                interpolate(linear(), number(get("keyName")), 18.0, literal("Text Variable Anchor Offset"))
            );
        }

        XCTAssertEqual(rawLayer->getTextVariableAnchorOffset(), propertyValue,
                       @"Setting textVariableAnchorOffset to a data expression should update text-variable-anchor-offset.");
        NSExpression *pedanticFunctionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:(CAST(keyName, 'NSNumber'), 'linear', nil, %@)", @{@18: constantExpression}];
        XCTAssertEqualObjects(layer.textVariableAnchorOffset, pedanticFunctionExpression,
                              @"textVariableAnchorOffset should round-trip data expressions.");
...

        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::VariableAnchorOffsetCollection>(
                interpolate(linear(), zoom(), 10.0, interpolate(linear(), number(get("keyName")), 18.0, literal("Text Variable Anchor Offset")))
            );
        }

        XCTAssertEqual(rawLayer->getTextVariableAnchorOffset(), propertyValue,
                       @"Setting textVariableAnchorOffset to a camera-data expression should update text-variable-anchor-offset.");
        pedanticFunctionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: pedanticFunctionExpression}];
        XCTAssertEqualObjects(layer.textVariableAnchorOffset, pedanticFunctionExpression,
                              @"textVariableAnchorOffset should round-trip camera-data expressions.");

        layer.textVariableAnchorOffset = nil;
        XCTAssertTrue(rawLayer->getTextVariableAnchorOffset().isUndefined(),
                      @"Unsetting textVariableAnchorOffset should return text-variable-anchor-offset to the default value.");
        XCTAssertEqualObjects(layer.textVariableAnchorOffset, defaultExpression,
                              @"textVariableAnchorOffset should return the default value after being unset.");
    }
    ...
    // text-writing-mode
    {
        ...
        nbgl::style::PropertyValue<std::vector<nbgl::style::TextWritingModeType>> propertyValue = { { nbgl::style::TextWritingModeType::Horizontal, nbgl::style::TextWritingModeType::Vertical } };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<std::vector<nbgl::style::TextWritingModeType>>(
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textWritingModes = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textWritingModes = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // icon-color
    {
        ...
        NGLTransition iconColorTransition = layer.iconColorTransition;
    ...
    // icon-halo-blur
    {
        ...
        NGLTransition iconHaloBlurTransition = layer.iconHaloBlurTransition;
    ...
    // icon-halo-color
    {
        ...
        NGLTransition iconHaloColorTransition = layer.iconHaloColorTransition;
    ...
    // icon-halo-width
    {
        ...
        NGLTransition iconHaloWidthTransition = layer.iconHaloWidthTransition;
    ...
    // icon-opacity
    {
        ...
        NGLTransition iconOpacityTransition = layer.iconOpacityTransition;
    ...
    // icon-translate
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.iconTranslation = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.iconTranslation = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // icon-translate-anchor
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.iconTranslationAnchor = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.iconTranslationAnchor = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-color
    {
        ...
        NGLTransition textColorTransition = layer.textColorTransition;
    ...
    // text-halo-blur
    {
        ...
        NGLTransition textHaloBlurTransition = layer.textHaloBlurTransition;
    ...
    // text-halo-color
    {
        ...
        NGLTransition textHaloColorTransition = layer.textHaloColorTransition;
    ...
    // text-halo-width
    {
        ...
        NGLTransition textHaloWidthTransition = layer.textHaloWidthTransition;
    ...
    // text-opacity
    {
        ...
        NGLTransition textOpacityTransition = layer.textOpacityTransition;
    ...
    // text-translate
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textTranslation = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textTranslation = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // text-translate-anchor
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.textTranslationAnchor = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.textTranslationAnchor = functionExpression, NSException, NSInvalidArgumentException, @"NGLSymbolLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
...
- (void)testPropertyNames {
    ...
    [self testPropertyName:@"symbol-screen-space" isBoolean:YES];
    ...
    [self testPropertyName:@"text-variable-anchor-offset" isBoolean:NO];
...
- (void)testValueAdditions {
    XCTAssertEqual([NSValue valueWithNGLIconAnchor:NGLIconAnchorCenter].NGLIconAnchorValue, NGLIconAnchorCenter);
    XCTAssertEqual([NSValue valueWithNGLIconAnchor:NGLIconAnchorLeft].NGLIconAnchorValue, NGLIconAnchorLeft);
    XCTAssertEqual([NSValue valueWithNGLIconAnchor:NGLIconAnchorRight].NGLIconAnchorValue, NGLIconAnchorRight);
    XCTAssertEqual([NSValue valueWithNGLIconAnchor:NGLIconAnchorTop].NGLIconAnchorValue, NGLIconAnchorTop);
    XCTAssertEqual([NSValue valueWithNGLIconAnchor:NGLIconAnchorBottom].NGLIconAnchorValue, NGLIconAnchorBottom);
    XCTAssertEqual([NSValue valueWithNGLIconAnchor:NGLIconAnchorTopLeft].NGLIconAnchorValue, NGLIconAnchorTopLeft);
    XCTAssertEqual([NSValue valueWithNGLIconAnchor:NGLIconAnchorTopRight].NGLIconAnchorValue, NGLIconAnchorTopRight);
    XCTAssertEqual([NSValue valueWithNGLIconAnchor:NGLIconAnchorBottomLeft].NGLIconAnchorValue, NGLIconAnchorBottomLeft);
    XCTAssertEqual([NSValue valueWithNGLIconAnchor:NGLIconAnchorBottomRight].NGLIconAnchorValue, NGLIconAnchorBottomRight);
    XCTAssertEqual([NSValue valueWithNGLIconPitchAlignment:NGLIconPitchAlignmentMap].NGLIconPitchAlignmentValue, NGLIconPitchAlignmentMap);
    XCTAssertEqual([NSValue valueWithNGLIconPitchAlignment:NGLIconPitchAlignmentViewport].NGLIconPitchAlignmentValue, NGLIconPitchAlignmentViewport);
    XCTAssertEqual([NSValue valueWithNGLIconPitchAlignment:NGLIconPitchAlignmentAuto].NGLIconPitchAlignmentValue, NGLIconPitchAlignmentAuto);
    XCTAssertEqual([NSValue valueWithNGLIconRotationAlignment:NGLIconRotationAlignmentMap].NGLIconRotationAlignmentValue, NGLIconRotationAlignmentMap);
    XCTAssertEqual([NSValue valueWithNGLIconRotationAlignment:NGLIconRotationAlignmentViewport].NGLIconRotationAlignmentValue, NGLIconRotationAlignmentViewport);
    XCTAssertEqual([NSValue valueWithNGLIconRotationAlignment:NGLIconRotationAlignmentAuto].NGLIconRotationAlignmentValue, NGLIconRotationAlignmentAuto);
    XCTAssertEqual([NSValue valueWithNGLIconTextFit:NGLIconTextFitNone].NGLIconTextFitValue, NGLIconTextFitNone);
    XCTAssertEqual([NSValue valueWithNGLIconTextFit:NGLIconTextFitWidth].NGLIconTextFitValue, NGLIconTextFitWidth);
    XCTAssertEqual([NSValue valueWithNGLIconTextFit:NGLIconTextFitHeight].NGLIconTextFitValue, NGLIconTextFitHeight);
    XCTAssertEqual([NSValue valueWithNGLIconTextFit:NGLIconTextFitBoth].NGLIconTextFitValue, NGLIconTextFitBoth);
    XCTAssertEqual([NSValue valueWithNGLSymbolPlacement:NGLSymbolPlacementPoint].NGLSymbolPlacementValue, NGLSymbolPlacementPoint);
    XCTAssertEqual([NSValue valueWithNGLSymbolPlacement:NGLSymbolPlacementLine].NGLSymbolPlacementValue, NGLSymbolPlacementLine);
    XCTAssertEqual([NSValue valueWithNGLSymbolPlacement:NGLSymbolPlacementLineCenter].NGLSymbolPlacementValue, NGLSymbolPlacementLineCenter);
    XCTAssertEqual([NSValue valueWithNGLSymbolZOrder:NGLSymbolZOrderAuto].NGLSymbolZOrderValue, NGLSymbolZOrderAuto);
    XCTAssertEqual([NSValue valueWithNGLSymbolZOrder:NGLSymbolZOrderViewportY].NGLSymbolZOrderValue, NGLSymbolZOrderViewportY);
    XCTAssertEqual([NSValue valueWithNGLSymbolZOrder:NGLSymbolZOrderSource].NGLSymbolZOrderValue, NGLSymbolZOrderSource);
    XCTAssertEqual([NSValue valueWithNGLTextAnchor:NGLTextAnchorCenter].NGLTextAnchorValue, NGLTextAnchorCenter);
    XCTAssertEqual([NSValue valueWithNGLTextAnchor:NGLTextAnchorLeft].NGLTextAnchorValue, NGLTextAnchorLeft);
    XCTAssertEqual([NSValue valueWithNGLTextAnchor:NGLTextAnchorRight].NGLTextAnchorValue, NGLTextAnchorRight);
    XCTAssertEqual([NSValue valueWithNGLTextAnchor:NGLTextAnchorTop].NGLTextAnchorValue, NGLTextAnchorTop);
    XCTAssertEqual([NSValue valueWithNGLTextAnchor:NGLTextAnchorBottom].NGLTextAnchorValue, NGLTextAnchorBottom);
    XCTAssertEqual([NSValue valueWithNGLTextAnchor:NGLTextAnchorTopLeft].NGLTextAnchorValue, NGLTextAnchorTopLeft);
    XCTAssertEqual([NSValue valueWithNGLTextAnchor:NGLTextAnchorTopRight].NGLTextAnchorValue, NGLTextAnchorTopRight);
    XCTAssertEqual([NSValue valueWithNGLTextAnchor:NGLTextAnchorBottomLeft].NGLTextAnchorValue, NGLTextAnchorBottomLeft);
    XCTAssertEqual([NSValue valueWithNGLTextAnchor:NGLTextAnchorBottomRight].NGLTextAnchorValue, NGLTextAnchorBottomRight);
    XCTAssertEqual([NSValue valueWithNGLTextJustification:NGLTextJustificationAuto].NGLTextJustificationValue, NGLTextJustificationAuto);
    XCTAssertEqual([NSValue valueWithNGLTextJustification:NGLTextJustificationLeft].NGLTextJustificationValue, NGLTextJustificationLeft);
    XCTAssertEqual([NSValue valueWithNGLTextJustification:NGLTextJustificationCenter].NGLTextJustificationValue, NGLTextJustificationCenter);
    XCTAssertEqual([NSValue valueWithNGLTextJustification:NGLTextJustificationRight].NGLTextJustificationValue, NGLTextJustificationRight);
    XCTAssertEqual([NSValue valueWithNGLTextPitchAlignment:NGLTextPitchAlignmentMap].NGLTextPitchAlignmentValue, NGLTextPitchAlignmentMap);
    XCTAssertEqual([NSValue valueWithNGLTextPitchAlignment:NGLTextPitchAlignmentViewport].NGLTextPitchAlignmentValue, NGLTextPitchAlignmentViewport);
    XCTAssertEqual([NSValue valueWithNGLTextPitchAlignment:NGLTextPitchAlignmentAuto].NGLTextPitchAlignmentValue, NGLTextPitchAlignmentAuto);
    XCTAssertEqual([NSValue valueWithNGLTextRotationAlignment:NGLTextRotationAlignmentMap].NGLTextRotationAlignmentValue, NGLTextRotationAlignmentMap);
    XCTAssertEqual([NSValue valueWithNGLTextRotationAlignment:NGLTextRotationAlignmentViewport].NGLTextRotationAlignmentValue, NGLTextRotationAlignmentViewport);
    XCTAssertEqual([NSValue valueWithNGLTextRotationAlignment:NGLTextRotationAlignmentAuto].NGLTextRotationAlignmentValue, NGLTextRotationAlignmentAuto);
    XCTAssertEqual([NSValue valueWithNGLTextTransform:NGLTextTransformNone].NGLTextTransformValue, NGLTextTransformNone);
    XCTAssertEqual([NSValue valueWithNGLTextTransform:NGLTextTransformUppercase].NGLTextTransformValue, NGLTextTransformUppercase);
    XCTAssertEqual([NSValue valueWithNGLTextTransform:NGLTextTransformLowercase].NGLTextTransformValue, NGLTextTransformLowercase);
    XCTAssertEqual([NSValue valueWithNGLTextWritingMode:NGLTextWritingModeHorizontal].NGLTextWritingModeValue, NGLTextWritingModeHorizontal);
    XCTAssertEqual([NSValue valueWithNGLTextWritingMode:NGLTextWritingModeVertical].NGLTextWritingModeValue, NGLTextWritingModeVertical);
    XCTAssertEqual([NSValue valueWithNGLIconTranslationAnchor:NGLIconTranslationAnchorMap].NGLIconTranslationAnchorValue, NGLIconTranslationAnchorMap);
    XCTAssertEqual([NSValue valueWithNGLIconTranslationAnchor:NGLIconTranslationAnchorViewport].NGLIconTranslationAnchorValue, NGLIconTranslationAnchorViewport);
    XCTAssertEqual([NSValue valueWithNGLTextTranslationAnchor:NGLTextTranslationAnchorMap].NGLTextTranslationAnchorValue, NGLTextTranslationAnchorMap);
    XCTAssertEqual([NSValue valueWithNGLTextTranslationAnchor:NGLTextTranslationAnchorViewport].NGLTextTranslationAnchorValue, NGLTextTranslationAnchorViewport);
}
