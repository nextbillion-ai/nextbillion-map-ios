...
@interface NGLHeatmapLayerTests : NGLStyleLayerTests
@end

@implementation NGLHeatmapLayerTests
...
- (void)testPredicates {
    ...
    NGLHeatmapStyleLayer *layer = [[NGLHeatmapStyleLayer alloc] initWithIdentifier:@"layerID" source:source];
...
- (void)testProperties {
    ...
    auto rawLayer = static_cast<nbgl::style::HeatmapLayer*>(layer.rawLayer);
    ...
    // heatmap-intensity
    {
        ...
        XCTAssertThrowsSpecificNamed(layer.heatmapIntensity = functionExpression, NSException, NSInvalidArgumentException, @"NGLHeatmapLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        ...
        NGLTransition heatmapIntensityTransition = layer.heatmapIntensityTransition;
    ...
    // heatmap-opacity
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.heatmapOpacity = functionExpression, NSException, NSInvalidArgumentException, @"NGLHeatmapLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.heatmapOpacity = functionExpression, NSException, NSInvalidArgumentException, @"NGLHeatmapLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        // Transition property test
        ...
        NGLTransition heatmapOpacityTransition = layer.heatmapOpacityTransition;
    ...
    // heatmap-radius
    {
        ...
        NGLTransition heatmapRadiusTransition = layer.heatmapRadiusTransition;
