...
@interface NGLLineLayerTests : NGLStyleLayerTests
@end

@implementation NGLLineLayerTests
...
- (void)testPredicates {
    ...
    NGLLineStyleLayer *layer = [[NGLLineStyleLayer alloc] initWithIdentifier:@"layerID" source:source];
...
- (void)testProperties {
    ...
    auto rawLayer = static_cast<nbgl::style::LineLayer*>(layer.rawLayer);
    ...
    // line-cap
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::LineCapType> propertyValue = { nbgl::style::LineCapType::Square };
...

        {
            ...
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::LineCapType>(
        ...
        XCTAssertThrowsSpecificNamed(layer.lineCap = functionExpression, NSException, NSInvalidArgumentException, @"NGLLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    ...
    // line-join
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::LineJoinType> propertyValue = { nbgl::style::LineJoinType::Miter };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<nbgl::style::LineJoinType>(
    ...
    // line-miter-limit
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.lineMiterLimit = functionExpression, NSException, NSInvalidArgumentException, @"NGLLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.lineMiterLimit = functionExpression, NSException, NSInvalidArgumentException, @"NGLLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // line-round-limit
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.lineRoundLimit = functionExpression, NSException, NSInvalidArgumentException, @"NGLLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.lineRoundLimit = functionExpression, NSException, NSInvalidArgumentException, @"NGLLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // line-blur
    {
        ...
        NGLTransition lineBlurTransition = layer.lineBlurTransition;
    ...
    // line-color
    {
        ...
        NGLTransition lineColorTransition = layer.lineColorTransition;
    ...
    // line-dasharray
    {
        ...
        nbgl::style::PropertyValue<std::vector<float>> propertyValue = { {1, 2} };
        ...
        {
            using namespace nbgl::style::expression::dsl;
            propertyValue = nbgl::style::PropertyExpression<std::vector<float>>(
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.lineDashPattern = functionExpression, NSException, NSInvalidArgumentException, @"NGLLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.lineDashPattern = functionExpression, NSException, NSInvalidArgumentException, @"NGLLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // line-gap-width
    {
        ...
        NGLTransition lineGapWidthTransition = layer.lineGapWidthTransition;
    ...
    // line-offset
    {
        ...
        NGLTransition lineOffsetTransition = layer.lineOffsetTransition;
    ...
    // line-opacity
    {
        ...
        NGLTransition lineOpacityTransition = layer.lineOpacityTransition;
    ...
    // line-pattern
    {
        ...
        nbgl::style::PropertyValue<nbgl::style::expression::Image> propertyValue = { "Line Pattern" };
        ...
        constantExpression = [NSExpression expressionWithFormat:@"NGL_FUNCTION('image', 'Line Pattern')"];
        XCTExpectFailure(@"Awaiting unit test refactoring https://github.com/maplibre/maplibre-native/issues/421");
        NSExpression *functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:($zoomLevel, %@, %@)", constantExpression, @{@18: constantExpression}];
        ...
        NGLTransition linePatternTransition = layer.linePatternTransition;
    ...
    // line-translate
    {
        ...
        functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
        XCTAssertThrowsSpecificNamed(layer.lineTranslation = functionExpression, NSException, NSInvalidArgumentException, @"NGLLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
        functionExpression = [NSExpression expressionWithFormat:@"ngl_step:from:stops:(bogus, %@, %@)", constantExpression, @{@18: constantExpression}];
        functionExpression = [NSExpression expressionWithFormat:@"ngl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)", @{@10: functionExpression}];
        XCTAssertThrowsSpecificNamed(layer.lineTranslation = functionExpression, NSException, NSInvalidArgumentException, @"NGLLineLayer should raise an exception if a camera-data expression is applied to a property that does not support key paths to feature attributes.");
    }
    ...
    // line-width
    {
        ...
        NGLTransition lineWidthTransition = layer.lineWidthTransition;
...
- (void)testValueAdditions {
    XCTAssertEqual([NSValue valueWithNGLLineCap:NGLLineCapButt].NGLLineCapValue, NGLLineCapButt);
    XCTAssertEqual([NSValue valueWithNGLLineCap:NGLLineCapRound].NGLLineCapValue, NGLLineCapRound);
    XCTAssertEqual([NSValue valueWithNGLLineCap:NGLLineCapSquare].NGLLineCapValue, NGLLineCapSquare);
    XCTAssertEqual([NSValue valueWithNGLLineJoin:NGLLineJoinBevel].NGLLineJoinValue, NGLLineJoinBevel);
    XCTAssertEqual([NSValue valueWithNGLLineJoin:NGLLineJoinRound].NGLLineJoinValue, NGLLineJoinRound);
    XCTAssertEqual([NSValue valueWithNGLLineJoin:NGLLineJoinMiter].NGLLineJoinValue, NGLLineJoinMiter);
    XCTAssertEqual([NSValue valueWithNGLLineTranslationAnchor:NGLLineTranslationAnchorMap].NGLLineTranslationAnchorValue, NGLLineTranslationAnchorMap);
    XCTAssertEqual([NSValue valueWithNGLLineTranslationAnchor:NGLLineTranslationAnchorViewport].NGLLineTranslationAnchorValue, NGLLineTranslationAnchorViewport);
}
